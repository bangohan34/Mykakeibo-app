...
if "subscriptions_auto_added" not in st.session_state:
    ...
    if added_count > 0:
        ...
        df = gsheets.load_kakeibo_data(worksheet)  # データを再読み込み
...
if not df.empty:
    ...
    if url_user_id == "u1" and not graph_df.empty:
        ...
        with tab_month:
            st.altair_chart(
                charts.create_combo_chart(graph_df, '年月', '%Y-%m', '%Y-%m', 0),
                use_container_width=True
            )
        # 週ごと（直近30週)
        with tab_week:
            ...
            if not df_30w.empty:
                st.altair_chart(
                    charts.create_combo_chart(df_30w, '週', '%m/%d', '%Y-%m-%d', -45),
                    use_container_width=True
                )
            else:
        ...
        with tab_day:
            ...
            if not df_30d.empty:
                st.altair_chart(
                    charts.create_combo_chart(df_30d, '日付', '%m/%d', '%Y-%m-%d', -45),
                    use_container_width=True
                )
            else:
...
if not df.empty:
    ...
    months_list = months_series[
        (months_series >= start_limit) &   # 2026年1月以降
    ...
    with st.expander("資産確認を開く", expanded=False):
        ...
        if diff != 0:
            ...
            if st.button("この差額を家計簿に記入する"):
                if diff > 0:
                    # 実際の方が多い → 収入として記入
                    gsheets.add_entry(worksheet, datetime.date.today(), '収入', 'その他', abs(diff), '資産調整')
                else:
                    # 実際の方が少ない → 支出として記入
                    gsheets.add_entry(worksheet, datetime.date.today(), '支出', 'その他', abs(diff), '資産調整')
